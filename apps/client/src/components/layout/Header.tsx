...
const linkBase =
  'bg-transparent border-0 text-[rgba(255,255,255,0.82)] text-[0.87rem] font-medium rounded-md cursor-pointer transition-colors no-underline inline-flex items-center font-[inherit] hover:text-white hover:bg-[rgba(255,255,255,0.12)] px-4 py-2.5 w-full md:w-auto md:px-[11px] md:py-[7px]';
const linkActive = 'text-white bg-[rgba(255,255,255,0.12)]';
    ...
    <header className="bg-[#0B2A4A] text-white px-5 h-[60px] flex items-center justify-between sticky top-0 z-[100] shadow-[0_2px_8px_rgba(11,42,74,0.3)]">
      ...
      >
        <div>
          ...
          <span className="block text-[0.68rem] text-white/60">
      ...
      <div className="flex items-center gap-0.5">
        ...
        <button
          ...
          aria-label="Menu"
        >
          <span className="block w-full h-0.5 bg-white rounded-sm" />
          <span className="block w-full h-0.5 bg-white rounded-sm" />
          <span className="block w-full h-0.5 bg-white rounded-sm" />
        </button>

        {/* Nav links: hidden on mobile until opened, always shown on desktop */}
        <div
          className={`${menuOpen ? 'flex' : 'hidden'} fixed top-[60px] left-0 right-0 z-[99] flex-col bg-[#0A3A67] p-2.5 gap-0.5 shadow-[0_8px_24px_rgba(0,0,0,0.3)] md:flex md:static md:flex-row md:bg-transparent md:p-0 md:gap-0.5 md:shadow-none md:z-auto`}
        >
          {!inCompany && (
          ...
          {inCompany && (
            <>
              ...
              <NavLink to={`/companies/${companyId}/restock`} className={({ isActive }) => `${linkBase}${isActive ? ` ${linkActive}` : ''}`} onClick={closeMenu}>🔄 Restock</NavLink>
          ...
          {inCompany && (
            <Link
              ...
              className="mt-1 w-full justify-center bg-[#19B37A] text-white border-0 text-[0.87rem] font-semibold px-[14px] py-[7px] rounded-md cursor-pointer no-underline inline-flex items-center font-[inherit] hover:bg-[#0F8F63] md:mt-0 md:w-auto md:ml-1.5"
